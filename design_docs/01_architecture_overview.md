...
| `3060/profile/llama2-7b_all_results.json` | `outputs/profiling/all_results/*.json` | One-file-per-device canonical raw result |
| `3060/profile/device_registry.json` | `outputs/profiling/registry/device_registry.v3.json` | Versioned normalized registry |
...
| `3060/role/pipeline.proto` | `pp_nextgen/runtime/proto/pipeline_v2.proto`（生成代码在 `pp_nextgen/runtime/grpc_gen/`） | `pipeline_stop`、`ring_return`、停止环与 worker0 语义 |
...
grpc_heterogeneous_pipeline_nextgen/
  ...
  outputs/runtime/logs/       # 默认 runtime 指标输出（run_cluster_sim）
  scheduler/
    export/
...
- **已实现**：`pp_nextgen` 下 gRPC Master/Worker、worker0 head+tail 双队列、按 designated 设备切分 `head_ordered_modules`/`tail_ordered_modules`、延迟 `pipeline_stop`、Windows 侧可配置的高精度 sleep 路径、指标默认写入 `outputs/runtime/logs/`。
