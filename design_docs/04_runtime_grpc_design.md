...
> 早期草稿中的部分 RPC 名（如 `PushStrategy`、`Heartbeat`）与标志名（如 `SHUTDOWN`）**未在 `pipeline_v2.proto` 中实现**。以 **`pp_nextgen/runtime/proto/pipeline_v2.proto`** 与 **`pp_nextgen/runtime/*`** 为准。  
...
- Per-worker **`RequestJournal`** 可导出 JSON（默认目录 **`outputs/runtime/logs/`**，由 `run_cluster_sim` 的 `--runtime-logs-dir` 配置）。
