...
| 生成 gRPC 桩 | `python tools/runtime/gen_runtime_proto.py`（需 `grpcio-tools`） |
| 求解策略并导出 worker JSON | `python tools/scheduler/solve_strategy.py --cluster <cluster.yaml>` |
| 本机 Master + 多 Worker 模拟 | `python tools/simulation/run_cluster_sim.py`（可选 `--runtime-logs-dir`，默认 `outputs/runtime/logs`） |
| 单独进程 | `python -m pp_nextgen.runtime.cli master ...` / `worker ...` |

**Worker 策略文件**（`outputs/scheduler/export/workers/*.strategy.json`）使用 **`head_ordered_modules` + `tail_ordered_modules`**；仅 **`designated_device`** 对应 worker 的 **`tail_ordered_modules` 为全局策略里最后 `designated_tail_n` 个模块**，其余机器 **`tail_ordered_modules` 为空**。兼容旧字段 **`ordered_modules`** 时由 `split_head_tail_modules_from_execution_plan` 解析。
...
6. **指标**：`run_cluster_sim` 默认将 **`*.runtime_metrics.json`** 写到 **`outputs/runtime/logs/`**。
...
| **`pp_nextgen/runtime/proto` 中未实现的 RPC** | 控制面扩展 | 如设计稿中的 `PushStrategy`、`Heartbeat`、流式 `PipeFrames` 等：**未**在 `pipeline_v2.proto` / Master 中实现 |
...
详细协议字段以 **`pp_nextgen/runtime/proto/pipeline_v2.proto`** 与 **`pp_nextgen/runtime/grpc_gen/*`** 为准。
