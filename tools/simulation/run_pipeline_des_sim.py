#!/usr/bin/env python3
"""Run pipeline discrete-event simulation and export metrics JSON."""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--out", default="outputs/runtime/simulation/pipeline_des.sim_metrics.json", help="Output JSON path")
    p.add_argument(
    ...
    p.add_argument(
        "--req-trace-out",
        default="",
        help="Optional path to export per-request DES trace JSON",
    )
    p.add_argument(
        "--stage-trace-out",
        default="",
        help="Optional path to export per-stage DES trace JSON",
    )
    return p.parse_args()
...
def main() -> int:
    ...
    print(f"Wrote DES metrics: {out_path}")
    print(
        json.dumps(
            {
