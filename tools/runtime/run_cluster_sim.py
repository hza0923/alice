...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--pipeline-strategy",
        type=str,
        default=str(root / "outputs" / "scheduler" / "export" / "pipeline_strategy.json"),
    )
    p.add_argument(
        "--workers-export-dir",
        type=str,
        default=str(root / "outputs" / "scheduler" / "export" / "workers"),
    )
    p.add_argument(
        "--runtime-logs-dir",
        type=str,
        default=str(root / "outputs" / "runtime" / "logs"),
        help="Where to write per-worker *.runtime_metrics.json (default: repo outputs/runtime/logs)",
    )
    p.add_argument(
        "--model-config",
        type=str,
